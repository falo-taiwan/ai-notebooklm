...
<body>
  <div class="card">
    <h2>地端管理閘道驗證</h2>
    <p>請輸入地端安全存取密碼以進入 FALO 控制台。</p>
    <form onsubmit="handleLogin(event)">
      <div class="field">
        <label for="password">安全密碼 (Password)</label>
        <div class="input-wrapper">
          <input type="password" id="password" placeholder="請輸入密碼" required autofocus>
...
def make_handler(config: AppConfig):
    class RuntimeHandler(BaseHTTPRequestHandler):
        ...
        def _is_authenticated(self) -> bool:
            ...
            return "falo_local_session=verified" in cookie_header
        ...
        def do_POST(self) -> None:
            ...
            if parsed.path == "/api/local-login":
                try:
                    ...
                    if password == "123456":
                        ...
                        self.send_header("Set-Cookie", "falo_local_session=verified; Path=/; Max-Age=86400; HttpOnly")
                        self.send_header("Content-Type", "application/json; charset=utf-8")
                        self.send_header("Content-Length", str(len(body)))
                        self.end_headers()
                        self.wfile.write(body)
                    else:
                        body = json.dumps({"ok": False, "error": "密碼錯誤！"}).encode("utf-8")
                        self.send_response(200)
                        self.send_header("Content-Type", "application/json; charset=utf-8")
                        self.send_header("Content-Length", str(len(body)))
                        self.end_headers()
            ...
            if parsed.path == "/api/simple-upload":
                ...
                task_id = task_queue_manager.add_task("notebooklm", user_name, payload)
                ...
                self._send_json({"ok": res.get("ok", False), "response": res})
                ...
                task_id = task_queue_manager.add_task("gemini", user_name, payload)
                detail = task_queue_manager.get_task_status_detail(task_id)
                self._send_json({
                    "ok": True,
                    "task_id": task_id,
        ...
        def do_GET(self) -> None:
            ...
            if not self._is_authenticated():
                if parsed.path in {"/", "/index.html", "/index-old.html"}:
            ...
            if parsed.path in {"/", "/index.html"}:
                ...
                if params.get("format", [""])[0] == "json":
                    self._send_json(list_notebooks(config))
                else:
