# AI NotebookLM Runtime Lab v3.01
...
- **設計精神**：官方 NotebookLM 在一個對話 ID 中只能鎖定一個特定的書庫。地端系統打破此限制，允許使用者在同一個對話 Session 中，隨時更換當前提問的書庫（下拉選單）。這提供了極佳的**交叉比對、對照查詢與綜合知識檢索**靈活性。
- **Turn-level 精細審計**：後台資料庫與審計模組不只在 Session 外層記錄書庫，更在每一個 `turns` 的提問節點上記錄當時發問時的 `notebook_id` 與 `notebook_title`。管理員的審計表格與匯出的 Excel 都會顯示對應書庫名稱，達到完善治理。
