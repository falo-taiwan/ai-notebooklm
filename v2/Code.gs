...
function setupSheet() {
  ...
  ensureSheet_(ss, SHEETS.SETTINGS, HEADERS.Settings, [
    ...
    ['passive_mode', 'true', 'Disable active ngrok/UrlFetch calls. Rely only on地端輪詢.'],
    ...
    ['runtime_report_method', '', 'Last reported connection method (manual/scheduled).'],
...
function doPost(e) {
  try {
    ...
    if (action === 'login') return json_(login_(payload.user_id, payload.password));
    if (action === 'update_host_info') return json_(updateHostInfo_(payload));
...
function updateHostInfo_(payload) {
  ...
  updateSetting_('runtime_last_seen_at', payload.local_time || now_());
  return { ok: true, message: 'Host info updated successfully.' };
}

function uiLogin(userId, password) {
  return login_(userId, password);
}
...
function login_(userId, password) {
  const defaults = {
    ...
    'user': { role: 'user', display_name: 'Normal User' }
  };
  
  const settings = objectFromRows_(rows_(SHEETS.SETTINGS), 'key', 'value');
  const token = settings.api_token || DEFAULT_TOKEN;
...
function wakeLocalWorker_(context) {
  ...
  const settings = objectFromRows_(rows_(SHEETS.SETTINGS), 'key', 'value');
  const passiveMode = String(settings.passive_mode || 'true').toLowerCase() === 'true';
  if (passiveMode) {
    const res = { ok: true, mode: 'wake_local_worker', passive: true, message: 'Passive mode active: waiting for local worker polling.' };
    log_(context.task_id || '', 'cloud_worker', 'wake_local_worker_passive', 'Task queued. Passive mode is enabled; waiting for local worker to poll.', res);
    return res;
  }
...
function renderPortal_() {
  ...
  const reportedLanUrl = settings.runtime_reported_lan_url || '';
  const reportedHostname = settings.runtime_hostname || '';
  const reportedMethod = settings.runtime_report_method || '';
  const reportedInterval = settings.runtime_poll_interval_seconds || '';
  const reportedLastSeen = settings.runtime_last_seen_at || '';
  const passiveModeSetting = settings.passive_mode || 'true';
  const apiTokenSetting = settings.api_token || DEFAULT_TOKEN;
...
<body>
  <!-- Public Host Status Area -->
  <div style="max-width:960px; margin:20px auto 0; padding:0 18px;">
    <!-- Host IP Status Indicator Card -->
    <section class="card" id="host-status-section" style="display:none; border-left:7px solid #208454;">
      <h2>地端主機狀態 (Host IP & Status)</h2>
      <div style="display:flex; align-items:center; gap:12px; margin-bottom:12px;">
        <span id="cloud-host-dot" style="width:12px; height:12px; border-radius:50%; display:inline-block; background-color:currentColor;"></span>
        <strong id="cloud-host-status-text">偵測中...</strong>
      </div>
      <div id="cloud-host-info" style="display:none; font-size:0.92rem;">
        <p style="margin:4px 0;"><strong>主機名稱:</strong> <span id="cloud-host-name">-</span></p>
        <p style="margin:4px 0;"><strong>更新方式:</strong> <span id="cloud-host-method">-</span></p>
        <p style="margin:4px 0;"><strong>最後回報時間:</strong> <span id="cloud-host-last-seen">-</span></p>
        <div style="display:flex; align-items:center; gap:8px; margin-top:8px;">
          <input id="cloud-host-url" type="text" readonly style="width:min(400px, 100%); background:#eef4f1; border-color:#cfd8d3; cursor:default; padding:10px; border-radius:6px; border:1px solid #cfd8d3;" value="">
          <button id="cloud-host-copy-btn" onclick="copyCloudHostUrl()" style="margin:0; padding:10px 16px; font-size:0.85rem; border:0; border-radius:8px; background:#1e725f; color:#fff; font-weight:700; cursor:pointer;">📋 複製網址</button>
        </div>
      </div>
      
      <!-- Developer diagnostic view when offline + admin role -->
      <div id="dev-diagnostic-panel" style="display:none; border-top:1px dashed #d8e1dd; margin-top:12px; padding-top:12px; font-size:0.85rem; color:#933333;">
        <h3 style="margin:0 0 6px; color:#933333;">🔧 開發者除錯診斷資訊</h3>
        <p id="dev-diagnostic-reason" style="margin:4px 0; font-weight:bold;"></p>
        <p style="margin:4px 0;">請確認以下幾點：</p>
        <ol style="margin:4px 0 8px; padding-left:20px; line-height:1.5;">
          <li>本機地端的 <code>FALO_Runtime</code> 服務是否正常執行？</li>
          <li>地端 <code>config/gas_config.json</code> 中的 <code>enabled</code> 與 <code>auto_poll_enabled</code> 是否均已設為 <code>true</code>？</li>
          <li>本機與雲端的 <code>api_token</code> 設定是否一致？</li>
        </ol>
        <details style="margin-top:8px;">
          <summary style="cursor:pointer; color:#1e725f; font-weight:bold;">檢視最後上報的原始 JSON 封包</summary>
          <pre id="dev-diagnostic-json" style="font-size:0.75rem; margin-top:6px; background:#fff8f8; border:1px solid #f2dede; padding:10px; border-radius:6px; overflow:auto;"></pre>
        </details>
      </div>
    </section>
  </div>
  ...
  <!-- Login wrapper -->
  <div id="login-wrapper" style="max-width:460px; margin:40px auto; padding:18px; display:none;">
    <div class="card" style="padding:28px; border-radius:12px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); background:#fff; border:1px solid #d8e1dd;">
      <h2 style="margin-top:0; text-align:center; color:#155343;">AI Gateway 雲端中控登入</h2>
      <p class="muted" style="text-align:center; margin-bottom:24px; font-size:0.9rem;">請輸入您的同仁帳密以存取中控台</p>
      
      <div class="field" style="margin-bottom:14px;">
        <label for="login_user" style="display:block; font-weight:700; margin-bottom:4px;">帳號 (User ID)</label>
        <input id="login_user" type="text" placeholder="例如 admin" style="width:100%; box-sizing:border-box; padding:10px; border:1px solid #cfd8d3; border-radius:6px;">
      </div>
      <div class="field" style="margin-bottom:20px;">
        <label for="login_pass" style="display:block; font-weight:700; margin-bottom:4px;">密碼 (Password)</label>
        <input id="login_pass" type="password" placeholder="請輸入密碼" style="width:100%; box-sizing:border-box; padding:10px; border:1px solid #cfd8d3; border-radius:6px;">
      </div>
      
      <button onclick="submitLogin(this)" style="width:100%; margin:0 0 16px 0; padding:12px; font-size:1rem; border:0; border-radius:8px; background:#1e725f; color:#fff; font-weight:700; cursor:pointer;">登入系統</button>
      <div id="login-error-msg" style="color:#933333; text-align:center; font-weight:bold; margin-bottom:12px; font-size:0.88rem;"></div>
      
      <div style="background:#eef4f1; border-radius:8px; padding:12px; font-size:0.8rem; color:#657286; border-left:4px solid #1e725f; line-height:1.5;">
        <strong style="color:#155343; display:block; margin-bottom:4px;">💡 預設測試帳密提示 (密碼均為 123456)：</strong>
        <ul style="margin:0; padding-left:16px;">
          <li>管理員：<code>admin</code></li>
          <li>特權用戶：<code>power</code></li>
          <li>一般同仁：<code>user</code></li>
        </ul>
      </div>
    </div>
  </div>
...
// Run immediately on script load to avoid window.onload not firing in Google's sandbox iframe (moved to the end of the script to avoid TDZ error)
...
function submitLogin(btn) {
  ...
  setButtonState(btn, 'is-running', '登入中...');
  google.script.run
    .withSuccessHandler(data => {
      setButtonState(btn, null, '登入系統');
      if (data && data.ok) {
    ...
    .withFailureHandler(err => {
      setButtonState(btn, null, '登入系統');
      errorEl.textContent = '系統錯誤: ' + String(err && err.message ? err.message : err);
    })
    .uiLogin(user, pass);
}
...
function renderHostStatus(role) {
  ...
  try {
    if (!REPORTED_LAST_SEEN) {
      ...
      text.textContent = '🔴 地端主機未上報任何連線資訊';
      info.style.display = 'none';
      
      if (role === 'admin') {
        const diagPanel = document.getElementById('dev-diagnostic-panel');
        const diagReason = document.getElementById('dev-diagnostic-reason');
        const diagJson = document.getElementById('dev-diagnostic-json');
        if (diagPanel) diagPanel.style.display = 'block';
        if (diagReason) diagReason.textContent = '異常原因：雲端未曾收到任何地端主機上報之狀態。';
        if (diagJson) diagJson.textContent = '無上報資料';
      }
      return;
    }
    
    // Parse last seen time robustly
    let lastSeenDate;
    try {
      lastSeenDate = new Date(REPORTED_LAST_SEEN);
      // Fallback for older engines or Safari if the above yielded Invalid Date
      if (isNaN(lastSeenDate.getTime())) {
        const lastSeenStr = REPORTED_LAST_SEEN.replace(/-/g, '/');
        lastSeenDate = new Date(lastSeenStr);
      }
    } catch (dateErr) {
      lastSeenDate = new Date(NaN);
    }
    ...
    if (isNaN(diffMins)) {
      diffMins = 999999; // Treat as offline if date is invalid
    }
    ...
    const nameEl = document.getElementById('cloud-host-name');
    if (nameEl) nameEl.textContent = REPORTED_HOSTNAME || '未知主機';
    ...
    if (REPORTED_METHOD === 'manual') {
      ...
      methodStr = '⏱️ 定時自動上報 (每 ' + mins + ' 分鐘)';
    }
    const methodEl = document.getElementById('cloud-host-method');
    if (methodEl) methodEl.textContent = methodStr;
    
    const lastSeenEl = document.getElementById('cloud-host-last-seen');
    if (lastSeenEl) {
      const displayDiff = diffMins === 999999 ? '未知/無法計算' : '約 ' + diffMins + ' 分鐘前';
      lastSeenEl.textContent = REPORTED_LAST_SEEN + ' (' + displayDiff + ')';
    }
    
    const urlEl = document.getElementById('cloud-host-url');
    if (urlEl) urlEl.value = REPORTED_LAN_URL;
    ...
    if (isOnline) {
      ...
      if (role === 'admin') {
        ...
        if (diagPanel) diagPanel.style.display = 'block';
        if (diagReason) {
          const displayMins = diffMins === 999999 ? '未知' : diffMins;
          diagReason.textContent = '異常原因：已逾期 ' + displayMins + ' 分鐘未收到回報（預估週期: ' + Math.round(intervalSec / 60) + ' 分鐘，容許閥值: ' + thresholdMins + ' 分鐘）。';
        }
        if (diagJson) {
          const rawJsonObj = {
            ...
            parsed_last_seen: lastSeenDate.toString(),
            diff_mins: diffMins
          };
          diagJson.textContent = JSON.stringify(rawJsonObj, null, 2);
        }
      }
    }
  } catch (err) {
    console.error('Exception inside renderHostStatus:', err);
    text.textContent = '⚠️ 載入錯誤: ' + err.message;
  }
}
...
const REPORTED_LAN_URL = '${reportedLanUrl}';
const REPORTED_HOSTNAME = '${reportedHostname}';
const REPORTED_METHOD = '${reportedMethod}';
const REPORTED_INTERVAL = '${reportedInterval}';
const REPORTED_LAST_SEEN = '${reportedLastSeen}';
const PASSIVE_MODE_SETTING = '${passiveModeSetting}';
const SYSTEM_API_TOKEN = '${apiTokenSetting}';
